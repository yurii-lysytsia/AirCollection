...
extension TableViewControllerProtocol where Self: TextViewControllerProtocol {

    public func textViewShouldBeginEditing(_ textView: UITextView) -> Bool {
        guard let indexPath = self.indexPathForRow(with: textView) else {
            assertionFailure("\(#function) called but `textView` didn't find on `tableView` rows. Please don't use this method for other destination")
            return true
        }
