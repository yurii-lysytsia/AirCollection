...
public enum CollectionViewElementKindSection: String {
    ...
    public init(rawValue: String) {
        ...
        case UICollectionView.elementKindSectionFooter:
            self = .footer
        default:
            fatalError("\(String(describing: CollectionViewElementKindSection.self)) can't initialize `init(rawValue:)` with raw value `\(rawValue)`. Please check and use only  `UICollectionView.elementKindSectionHeader` or `UICollectionView.elementKindSectionFooter`")
        }
    }
